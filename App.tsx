...
import { DadosAvaliacao, Notas, Dimensao } from './types';
import { gerarMarkdown, gerarPDF } from './services/exportService';
...
export default function App() {
    ...
    if (saved) {
      try {
        ...
        if (savedDados.nomeColaborador && Object.keys(savedNotas).length > 0) {
          const load = window.confirm('Encontramos uma avaliação em andamento. Deseja continuar de onde parou?');
          if (load) {
            setDados(savedDados);
            setNotas(savedNotas);
            setStep(1);
  ...
  const handleExportMD = () => {
    try {
      ...
      alert('Erro ao gerar arquivo Markdown. Tente novamente.');
    }
  };
  ...
  const limparRascunho = () => {
    if (window.confirm('Deseja realmente limpar todos os dados e começar uma nova avaliação?')) {
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 print:bg-white print:p-0">
        <div className="max-w-4xl mx-auto print:max-w-none print:w-full print:mx-0">
           <button onClick={() => setShowReport(false)} className="mb-4 px-4 py-2 bg-gray-200 rounded-lg text-sm font-medium hover:bg-gray-300 print:hidden no-print">
            ← Voltar para Edição
          </button>
